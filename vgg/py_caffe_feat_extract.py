...
def caffe_extract_feats(path_imgs, path_model_def, path_model, layer, WITH_GPU = True, batch_size = 10):
    '''
...
if __name__ == '__main__':
    ...
    parser.add_argument('--layer', default='fc7', help='layer to extract features from')
    ...
    path_imgs = [ os.path.expanduser(file) for file in imgs ]
    
    start_time = time.time()
    print "Feature Extraction for %d images starting now"%(len(path_imgs))
    feats = caffe_extract_feats(path_imgs, path_model_def_file, path_model, args.layer, WITH_GPU)
